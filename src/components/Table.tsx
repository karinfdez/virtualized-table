...
export const Table = () => {
	...
	const [total, setTotal] = useState(0) //The total amount of products on the API
...
// Initial load
	...
	useEffect(() => {
		// If no more data, activate state for 3 seconds to show message with no more data
  if (!hasMore) {
    setShowNoMore(true);
   ...
   return (
		<>
			{data && data.length > 0 && (
				<>
					<div className="w-full mx-auto flex justify-center py-8 text-indigo-700 font-medium">{`Showing ${data.length}/${total} products`}</div>
